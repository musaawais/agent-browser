  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const input: AgentTaskInput = {
        name: form.name || `Task ${new Date().toLocaleTimeString()}`,
      ...
      const task = await api.agent.createTask(input);
      onCreated(task);
    } finally {
      setCreating(false);
    }
  ...
  return (
    <div className="glass rounded-xl p-4 flex flex-col gap-3">
      ...
      <div>
        ...
        <input className="input-dark" placeholder="e.g. Research competitor pricing"
      ...
      <div>
        ...
        <input className="input-dark" placeholder="e.g. best SEO tools 2024"
          value={form.keyword} onChange={(e) => set('keyword', e.target.value)} />
      </div>

      {/* URLs */}
      <div>
        <label className="form-label">Target URLs <span style={{ color: 'rgba(255,255,255,0.35)' }}>(one per line)</span></label>
        <textarea
      ...
      <div>
        ...
        <select className="input-dark" value={form.countryCode} onChange={(e) => set('countryCode', e.target.value)}
      ...
      <div>
        <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 12, color: 'rgba(255,255,255,0.7)', fontWeight: 500 }}>
          <input type="checkbox" checked={form.useProxy} onChange={(e) => set('useProxy', e.target.checked)}
            style={{ accentColor: '#6366f1' }} />
          Use custom proxy
        </label>
        {form.useProxy && (
          <div className="flex gap-2 mt-2">
            <input className="input-dark flex-1" placeholder="Host" value={form.customProxyHost}
              onChange={(e) => set('customProxyHost', e.target.value)} />
            <input className="input-dark" placeholder="Port" type="number" value={form.customProxyPort}
              onChange={(e) => set('customProxyPort', parseInt(e.target.value))}
              style={{ width: 70 }} />
            <select className="input-dark" value={form.customProxyProtocol}
              onChange={(e) => set('customProxyProtocol', e.target.value)} style={{ width: 90, appearance: 'auto' }}>
      ...
      <div className="glass rounded-lg p-3 flex flex-col gap-2">
        <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 12, color: 'rgba(255,255,255,0.7)', fontWeight: 500 }}>
          <input type="checkbox" checked={form.clickInternalLinks} onChange={(e) => set('clickInternalLinks', e.target.checked)}
            style={{ accentColor: '#6366f1' }} />
        ...
        {form.clickInternalLinks && (
          <div>
            ...
            <input type="number" className="input-dark" min={1} max={20} value={form.maxInternalLinks}
      ...
      <div className="flex gap-2 mt-1">
        ...
        <button className="btn-primary flex-1" onClick={handleSubmit} disabled={creating || !form.urls.trim()}>
